...
s = [p['item_price'] for p in events if p['eventType'] == 'itemViewEvent' or p['eventType'] == 'itemBuyEvent']
p = [(i, s.count(i)) for i in s]
z = [key['eventType'] for key in events]

sumin = 0
for key in events:
    if key['firstInSession']:
        print(key['pageViewId'])
